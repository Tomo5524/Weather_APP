...
def format_response(weather_json):
    try:
        ...
        final_str = 'City: %s \nConditions: %s \n Temperature: %s°C' % (city, conditions, round(int(temp)))
    except:
...
jpg_image = ImageTk.PhotoImage(Image.open("BG_worldmap.jpg"))
#background_image= tk.PhotoImage(file='./background.jpg')
background_label = tk.Label(app, image=jpg_image)
...
city_input = tk.Entry(frame, font=40)
city_input.place(relwidth=0.65, relheight=1)

submit = tk.Button(frame, text='Search', font=40, command=lambda: get_weather(city_input.get()))
#submit.config(font=)
